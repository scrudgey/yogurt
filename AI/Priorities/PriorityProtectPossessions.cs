...
namespace AI {
    public class PriorityProtectPossessions : Priority {
        ...
        public PriorityProtectPossessions(GameObject g, Controller c) : base(g, c) {
            priorityName = "possessions";
            Goal findPossession = new GoalWander(gameObject, control);
            findPossession.successCondition = new ConditionSawObjectRecently(gameObject, possession);
            ...
            returnObject.successCondition = new ConditionObjectInPlace(gameObject, possession, returnPosition);
